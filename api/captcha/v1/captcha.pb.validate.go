...
// source: api/captcha/v1/captcha.proto
...
func (m *VerifyCaptchaRequest) _validateHostname(host string) error {
	s := strings.ToLower(strings.TrimSuffix(host, "."))
...
func (m *VerifyCaptchaRequest) _validateEmail(addr string) error {
	a, err := mail.ParseAddress(addr)
	if err != nil {
		return err
	}
